document.addEventListener("DOMContentLoaded", () => {
    ...
    let activeDrag = null;
    let dragOffset = 0;
    let shouldAutoStopAtSegmentEnd = false;

    const analysedSegments = [];
    ...
    function renderTimelineMarkers(duration) {
        ...
        analysedSegments.forEach((segment, index) => {
            ...
            marker.addEventListener("click", () => {
                ...
                shouldAutoStopAtSegmentEnd = true;
                videoPlayer.pause();
    ...
    videoPlayer.addEventListener("timeupdate", () => {
        ...
        if (
            shouldAutoStopAtSegmentEnd &&
            !videoPlayer.paused &&
            videoPlayer.currentTime >= end
        ) {
            videoPlayer.pause();
            shouldAutoStopAtSegmentEnd = false;
        }
    });
    ...
    document.addEventListener("mousemove", (event) => {
        if (!activeDrag || !videoPlayer.duration) return;
